<template>
  <v-card @dblclick="getCursor">
    <v-toolbar dark class="secondary" fluid dense>
      ...
      <v-toolbar-title class="body-2 grey--text">Active date:
    ...
    <vue-cal style="min-height: 460px;" default-view="week" :time-from="8 * 60" :time-to="22 * 60" :disable-views="['years', 'year', 'day']" :events="events" events-on-month-view="short" hide-view-selector show-all-day-events="true" startWeekOnSunday :selected-date="selectedDate" @cell-click="cellClick" @cell-dblclick="cellDbClick" @view-change="viewChange">
          ...
          <template v-slot:activator="{ on }">
            <v-card v-on="on" flat class="white--text pa-1 text-xs-left" :color="event.color" style="height: 100%; margin-left: 1px !important; margin-bottom: 1px !important; cursor: pointer;">
              <div class="text-truncate condensed">
                ...
                <span v-if="view != 'month' && !event.allDay" class="right caption">
                  <v-icon small color="yellow">fa-star</v-icon>{{event.totalUnits}}
                </span>
              </div>
              <template v-if="!event.allDay && view != 'month'">
                ...
                <template v-if="event.totalUnits > 4">
                  <small class="vuecal__event-time text-truncate condensed" v-if="view.id != 'month'">
                    <v-icon small color="white">fa-clock</v-icon>&nbsp;<span>{{ event.start  | moment('LT') }}</span> - <span>{{ event.end  | moment('LT') }}</span>
                  </small><br>
                  <div class="text-truncate condensed">
