  ...
  computed: {
    filteredSessions: function () {
      return this.sessions.filter(item => {
        ...
        return (
          (item.userFullname ? item.userFullname.match(regex) : true) ||
          (item.clientFullname ? item.clientFullname.match(regex) : true) ||
          (item.sessionStatus ? item.sessionStatus.match(regex) : true)
        );
      });
    },
  },
